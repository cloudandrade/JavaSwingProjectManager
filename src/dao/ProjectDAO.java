...
public class ProjectDAO {
	...
	private static String ADD_PROJECT = "INSERT INTO PROJECT "
			+ "(title, description, owner_id, ods_id, created_at, status) " + "VALUES (?,?,?,?,?,?)";
	...
	private static String LIST_PROJECTS_OWNERS_ODS = "SELECT "
			...
			+ "U.name as owner_name, U.phone as owner_phone, " + "O.title as ods_name " + " FROM PROJECT P "
			+ "JOIN USER U ON P.owner_id = U.id " + "JOIN ODS O ON P.ods_id = O.id ";

	private static String FIND_PROJECT = "SELECT "
			+ "P.id, P.title, P.description, P.owner_id, P.ods_id, P.created_at, P.status, "
			+ "U.name as owner_name, U.phone as owner_phone, " + "O.title as ods_name " + " FROM PROJECT P "
			+ "JOIN USER U ON P.owner_id = U.id " + "JOIN ODS O ON P.ods_id = O.id " + " AND P.ID = ? ";

	private static String UPDATE_PROJECT = "UPDATE PROJECT SET " + "title = ?, description = ?, ods_id = ?, status = ? "
			+ "WHERE id = ?";

	private static String DELETE_PROJECT = "DELETE FROM PROJECT " + " WHERE ID = ? ";
	...
	public ArrayList<ProjectModel> findByTitle(String search) {
		...
		String FIND_PROJECT_BY_TITLE = "SELECT "
				+ "P.id, P.title, P.description, P.owner_id, P.ods_id, P.created_at, P.status, "
				+ "U.name as owner_name, U.phone as owner_phone, " + "O.title as ods_name " + " FROM PROJECT P "
				+ "JOIN USER U ON P.owner_id = U.id " + "JOIN ODS O ON P.ods_id = O.id " + "AND P.title LIKE '%"
				+ search + "%'";
		try {
